...
def build_ds_from_notes(notes_with_phoneme, output_path: Path):
    """
    Build a DiffSinger .ds JSON file from note-level data with user-specified phonemes.
    ...
    note_dur_seq = [note['duration'] for note in notes_with_phoneme]
    ph_seq = [note['phoneme'] for note in notes_with_phoneme]
    # Default slur sequence to 0 if not provided
    is_slur_seq = [note.get('is_slur', 0) for note in notes_with_phoneme]
